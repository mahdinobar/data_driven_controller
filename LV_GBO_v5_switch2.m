% GBO version 5 switch 2 ratio of uncertainty
...
dir=append(tmp_dir,'\GBO_sw2_v5_', string(expr), '\');
if not(isfolder(dir))
    mkdir(dir)
end
%% load gain limits
...
elseif LVswitch==0  % LVswitch==0 means we need to decide to call either real or surrogate to get data
    [~,~,Trace, LVgains,hyper_grid,idx_G2, G2, counter_s,when_switch_s, debug_ratio, ys] = LV_bayesoptGPML_v5_switch2(fun,opt,hyper_grid,counter_s, G2data,idx_G2,when_switch_s,counter_real);
